...
serve(async (req) => {
  ...
  try {
    ...
    const prompt = `You are a job search assistant specializing in tech jobs in India. Generate realistic job listings based on current market trends. Include a mix of full-time positions and internships from real companies. For each job, generate a valid LinkedIn job search URL.

Generate 8-10 realistic job listings for: ${query}
